  ...
  <div class="main box_center cf">
    <div class="userBox cf">
      ...
      <div class="user_r">
        ...
        <p class="github-login" @click="loginWithGitHub">
          <img src="../assets/images/github-fill.png" alt="GitHub" class="github-icon" />
        </p>
  ...
  setup() {
    ...
    const loadImgVerifyCode = async () => {
      try {
        ...
        // 检查 data.img 是否已经包含了 'data:image/png;base64,' 前缀
        if (data && data.img) {
          if (data.img.startsWith("data:image/png;base64,")) {
            state.imgVerifyCode = data.img;
          } else {
            state.imgVerifyCode = "data:image/png;base64," + data.img;
          }
          state.sessionId = data.sessionId;
        } else {
          console.error("Invalid image data received:", data.img);
          state.imgVerifyCode = "";
          state.sessionId = "";
        }
      } catch (error) {
        console.error("Failed to load image verification code:", error);
        state.imgVerifyCode = "";
        state.sessionId = "";
      }
    };
    ...
    const registerUser = async () => {
      ...
      if (!/^[a-zA-Z0-9]{4}$/.test(state.velCode)) {
    ...
    const loginWithGitHub = () => {
      const client_id = 'Ov23liGGri5j7mkKl1GA';
      const redirect_uri = 'http://localhost:1024'; // 这里使用主页面作为回调地址
      const authorize_uri = 'https://github.com/login/oauth/authorize';

      // 构建GitHub OAuth URL并重定向用户
      window.location.href = `${authorize_uri}?client_id=${client_id}&redirect_uri=${encodeURIComponent(redirect_uri)}`;
    };
