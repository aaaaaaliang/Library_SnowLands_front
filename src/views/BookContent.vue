      ...
      <div class="readMain cf">
        <div class="read_menu">
          <div class="menu_left" style="background-color: rgba(255, 255, 255, 0.45)">
            <ul>
              <li>
                <a class="ico_catalog" @click="chapterList(data.chapterInfo.bookId)" href="javascript:void(0)" title="目录">
                  <b>目录</b>
                </a>
              </li>
              <li>
                <a class="ico_page" @click="bookDetail(data.chapterInfo.bookId)" href="javascript:void(0)" title="返回书页">
                  <b>书页</b>
                </a>
              </li>
            </ul>
          ...
          <div class="menu_right" style="position: fixed; bottom: 0">
            <ul>
              <li>
                <a class="ico_pagePrev" @click="preChapter(data.chapterInfo.bookId)" href="javascript:void(0)" title="上一章">
                  <i>上一章</i>
                </a>
              </li>
              <li>
                <a class="ico_pageNext" @click="nextChapter(data.chapterInfo.bookId)" href="javascript:void(0)" title="下一章">
                  <i>下一章</i>
                </a>
              </li>
            </ul>
            ...
            <div class="textbox cf" style="background-color: rgba(255, 255, 255, 0.45)">
              <div class="book_title">
                ...
                <div class="textinfo" v-if="data.chapterInfo">
                  类别：{{ data.bookInfo.categoryName }} 作者：
                  <a href="javascript:searchByK('最终马甲')" v-if="data.bookInfo">{{ data.bookInfo.authorName }}</a>
              ...
              <div class="txtwrap">
                <div id="showReading" class="readBox" style="font-size: 16px; font-family: microsoft yahei;white-space:break-spaces" v-html="data.bookContent"></div>
              </div>
            </div>
          </div>
          <div class="nextPageBox">
            <a style="background-color: rgba(255, 255, 255, 0.45)" class="prev" href="javascript:void(0)" @click="preChapter(data.chapterInfo.bookId)">上一章</a>
            <a style="background-color: rgba(255, 255, 255, 0.45)" class="dir" @click="chapterList(data.chapterInfo.bookId)" href="javascript:void(0)">目录</a>
            <a style="background-color: rgba(255, 255, 255, 0.45)" class="next" href="javascript:void(0)" @click="nextChapter(data.chapterInfo.bookId)">下一章</a>
          </div>
        </div>
      </div>
    </div>
  ...
  setup() {
    ...
    const preChapter = async (bookId) => {
      try {
        ...
        if (data && data.id) {
          router.push({ path: `/book/${bookId}/${data.id}` });
          init(data.id);
        } else {
          ElMessage.warning("已经是第一章了！");
        }
      } catch (error) {
        ElMessage.error("获取上一章失败！");
      }
    };
